...
param (
  ...
  [ValidateSet('major', 'minor', 'patch', 'incr.')]
  [string]
  $version,
...
if ($tag -ne 'latest') {
  ...
  [string]$currentVersionString = npm.cmd dist-tag ls | Select-String $tag
  ...
  if ($selectedChoice -eq 3) {
    ...
    switch ($selectedChoice) {
      0 { $betaVersion = "$($major+1).0.0-$tag.0" }
      1 { $betaVersion = "$major.$($minor+1).0-$tag.0" }
      Default { $betaVersion = "$major.$minor.$($patch+1)-$tag.0" }
    }
  }

  Write-Host "New version is $betaVersion, please confirm:`r`nInput Y(yes) to confirm or input N(no) to exit. Default Y(yes)." -ForegroundColor Green
  $confirm = Read-Host
...
if ($null -eq $betaVersion) {
  npm.cmd version $version
}
else {
  npm.cmd version $betaVersion
}
